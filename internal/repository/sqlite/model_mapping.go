...
func (r *ModelMappingRepository) Create(mapping *domain.ModelMapping) error {
	...
	result, err := r.db.db.Exec(
		`INSERT INTO model_mappings (created_at, updated_at, client_type, provider_id, project_id, route_id, api_token_id, pattern, target, priority, is_enabled, is_builtin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		formatTime(mapping.CreatedAt), formatTime(mapping.UpdatedAt),
		mapping.ClientType, mapping.ProviderID, mapping.ProjectID, mapping.RouteID, mapping.APITokenID,
...
func (r *ModelMappingRepository) Update(mapping *domain.ModelMapping) error {
	...
	_, err := r.db.db.Exec(
		`UPDATE model_mappings SET updated_at = ?, client_type = ?, provider_id = ?, project_id = ?, route_id = ?, api_token_id = ?, pattern = ?, target = ?, priority = ?, is_enabled = ?, is_builtin = ? WHERE id = ?`,
...
func (r *ModelMappingRepository) GetByID(id uint64) (*domain.ModelMapping, error) {
	row := r.db.db.QueryRow(`SELECT id, created_at, updated_at, client_type, provider_id, project_id, route_id, api_token_id, pattern, target, priority, is_enabled, is_builtin FROM model_mappings WHERE id = ?`, id)
	return r.scanMapping(row)
}

func (r *ModelMappingRepository) List() ([]*domain.ModelMapping, error) {
	rows, err := r.db.db.Query(`SELECT id, created_at, updated_at, client_type, provider_id, project_id, route_id, api_token_id, pattern, target, priority, is_enabled, is_builtin FROM model_mappings ORDER BY priority, id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *ModelMappingRepository) ListEnabled() ([]*domain.ModelMapping, error) {
	rows, err := r.db.db.Query(`SELECT id, created_at, updated_at, client_type, provider_id, project_id, route_id, api_token_id, pattern, target, priority, is_enabled, is_builtin FROM model_mappings WHERE is_enabled = 1 ORDER BY priority, id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *ModelMappingRepository) ListByQuery(query *domain.ModelMappingQuery) ([]*domain.ModelMapping, error) {
	rows, err := r.db.db.Query(
		`SELECT id, created_at, updated_at, client_type, provider_id, project_id, route_id, api_token_id, pattern, target, priority, is_enabled, is_builtin
		FROM model_mappings
		WHERE is_enabled = 1
		...
		query.ClientType, query.ProviderID, query.ProjectID, query.RouteID, query.APITokenID,
	)
	if err != nil {
		return nil, err
	}
